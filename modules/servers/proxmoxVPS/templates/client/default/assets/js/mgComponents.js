        ...
        $("#groupList.vSortable").sortable(
            {
                ...
                stop: function(event, ui)
                {
                    ...
                    mgPageControler.vueLoader.updateSorting(order, 'addCategoryForm', 'ModulesGarden_ProxmoxVps_App_UI_Widget_DoeTldConfigComponents_CategoryForms_AddCategoryForm');
    ...
    $('#itemContentContainer tbody.vSortable').sortable(
        {
            stop: function(event, ui)
            {
                ...
                mgPageControler.vueLoader.updateSorting(order, 'assignTldForm', 'ModulesGarden_ProxmoxVps_App_UI_Widget_DoeTldConfigComponents_CategoryForms_AssignTldForm');
            },
...
function restoreFileNextPage(vueController, params, event) {
    vueController.loading = true;

    var clearUrl = window.location.href;
    if(window.location.href.indexOf('#') !== -1)
    {
        clearUrl = window.location.href.substr(0, window.location.href.indexOf('#'));
    }
    var url = clearUrl + '&mg-action=updateCurrentPathNext';
    $.post(url, {path : params[0]}).done(function(data){
        vueController.updateProjects();
    });
}

function restoreFilePageBack(vueController, params, event) {
    vueController.loading = true;
...
function restoreFileHomePage(vueController, params, event) {
    vueController.loading = true;
