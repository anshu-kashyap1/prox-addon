{**********************************************************************
* ProxmoxVps product developed. (2017-10-04)
* *
*
...
{if $rawObject->getConfirmMessage()}
    {if $rawObject->isTranslateConfirmMessage()}
        <div class="lu-m-b-2x">{$MGLANG->T($rawObject->getConfirmMessage())|unescape:'html'}</div>
    {else}
        <div class="lu-m-b-2x">{$rawObject->getConfirmMessage()|unescape:'html'}</div>
    {/if}
{/if}
