{**********************************************************************
* ProxmoxVps product developed. (2017-10-10)
*
*
...
<script type="text/x-template" id="t-mg-restoreFileDataTable-{$elementId|strtolower}"
...
>
    <div class="lu-widget widgetActionComponent vueDatatableTable {$rawObject->getClasses()}" id="{$rawObject->getId()}"
         namespace="{$namespace}" index="{$rawObject->getIndex()}" actionid="{$rawObject->getIndex()}">
        <div class="lu-t-c  datatableLoader" id="{$elementId}" data-table-container data-check-container>
                ...
                <div class="lu-widget__top lu-top">
                    <ul class="lu-breadcrumb lu-breadcrumb--angle-separator">
                        <li class="lu-breadcrumb__item">
                            <a class="lu-breadcrumb__link" href="javascript:;"><i class="lu-zmdi lu-zmdi-folder"></i></a>
                        </li>
                        <li class="lu-breadcrumb__item">
                            <a class="lu-breadcrumb__link" href="javascript:;"
                               @click="makeCustomAction('restoreFileHomePage', [],$event)">Home</a>
                        </li>
                        <li v-for="(pathPart,index) in additionalData.currentDirPath">
                        <span v-if="index != additionalData.currentDirPath.length - 1" class="lu-breadcrumb__item">
                            <a href="javascript:;" class="lu-breadcrumb__link"
                               {literal}@click="makeCustomAction('restoreFileHomePage', {'current':pathPart,'pathIndex':index},$event)">{{ pathPart }}{/literal}</a>
                        </span>
                            <span v-else class="lu-breadcrumb__item  is-active">
                            <span href="javascript:;" class="lu-breadcrumb__link" {literal} >{{ pathPart }}{/literal}</span>
                        </span>
                        </li>
                    </ul>
                </div>
            ...
            <div class="lu-widget__body">
                ...
                <div class="lu-t-c__mass-actions lu-top">
                    <div class="lu-top__title"><span
                                class="lu-badge lu-badge--primary lu-value">0</span> {$MGLANG->absoluteT('datatableItemsSelected')}
                    </div>
                    <div class="lu-top__toolbar">
                ...
                <div class="dataTables_wrapper no-footer">
                    <div>
                        <table class="lu-table lu-table--mob-collapsible dataTable no-footer dtr-column" width="100%"
                               role="grid">
                            <thead>
                            <tr role='row'>
                                    ...
                                    {if $rawObject->hasMassActionButtons() && $collArrKeys[0] === $tplKey}
                                        <th class="{if $tplValue->orderable}{$tplValue->orderableClass}{/if} {if $tplValue->class !== ''}{$tplValue->class}{/if}"
                                            ...
                                            {if $rawObject->isvSortable()}
                                                <span class="drag-and-drop-icon" style="visibility: hidden;">
                                                ...
                                                <div class="lu-form-check">
                                                    <label>
                                                        <input type="checkbox" data-check-all=""
                                                               class="lu-form-checkbox">
                                                        <span class="lu-form-indicator"></span>
                                                    </label>
                                                </div>
                                                <span class="lu-table__text"
                                                      {if $tplValue->orderable}v-on:click="updateSorting"{/if}>{if $tplValue->rawTitle}{$tplValue->rawTitle}{else}{$MGLANG->T('table', $tplValue->title)}{/if}</span>
                                            </div>
                                        </th>
                                    {else}
                                        <th class="{if $tplValue->orderable}{$tplValue->orderableClass}{/if} {if $tplValue->class !== ''}{$tplValue->class}{/if}" {if $tplValue->orderable} aria-sort="descending" {/if}
                                                {if $tplValue->orderable}v-on:click="updateSorting"{/if}
                                            name="{$tplValue->name}">
                            ...
                            <tr>
                                <td role="row">
                                    <div class="lu-d-flex lu-align-items-center">
                                        ...
                                        <a class="lu-m-l-1x" href="javascript:;" @click="makeCustomAction('restoreFilePageBack', [], $event)">..</a>
                                    </div>
                                </td>
                                <td></td>
                                <td></td>
                            ...
                            <tr v-for="dataRow in dataRows"
                                {literal}:actionid{/literal}="dataRow.{$rawObject->getActionIdColumnName()}" role="row">
                                {foreach from=$customTplVars.columns key=tplKey item=tplValue}
                                    {if $rawObject->hasMassActionButtons() && $collArrKeys[0] === $tplKey}
                                        <td>
                                            {if $rawObject->isvSortable()}
                                                <span class="drag-and-drop-icon">
                                            ...
                                            <div class="lu-rail">
                                                <div class="lu-form-check">
                                                    <label>
                                                        <input type="checkbox"
                                                               class="lu-form-checkbox table-mass-action-check"
                                                               {literal}:value="dataRow.id"{/literal}>
                    ...
                    {if $rawObject->isViewFooter()}{literal}
                        ...
                        <span v-for="pageNumber in pagesMap">
                                    <a v-if='pageNumber && pageNumber !== "..."'
                                       :class='"paginate_button" + (curPage === pageNumber ? " current" : "")'
                                       aria-controls="DataTables_Table_0" v-on:click="changePage"
                                       :data-dt-idx="pageNumber" tabindex="0"> {{ pageNumber}} </a>
                                    <a v-if='pageNumber && pageNumber === "..."' class="paginate_button disabled"> {{ pageNumber}} </a>
                                </span>
                        <a
                        :class='"paginate_button next" + ((curPage === allPages || allPages === 0) ? " disabled" : "")'
                        aria-controls="DataTables_Table_0" :data-dt-idx='curPage +1' tabindex="0" href="javascript:"
                        page="next" v-on:click="changePage" id="{/literal}{$elementId}{literal}_next"></a>
                        </div>
                        <div class="lu-dt-buttons">
                    {/literal}
                        {foreach from=$rawObject->getTableLengthList() key=tplKey item=lengthList}
                            {if $lengthList == "inf"}
                                <a class="dt-button {if $rawObject->getTableLength() == $lengthList}active{/if}"
                                   tabindex="0" data-length="999999" v-on:click="updateLength"
                                   aria-controls="DataTables_Table_0" href="#{$elementId}">
                                    <span>∞</span>
                                </a>
                            {else}
                                <a class="dt-button {if $rawObject->getTableLength() == $lengthList}active{/if}"
                                   tabindex="0" data-length="{$lengthList}" v-on:click="updateLength"
                                   aria-controls="DataTables_Table_0" href="#{$elementId}">
