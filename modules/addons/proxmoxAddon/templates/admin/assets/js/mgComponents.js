...
function initModalSelects() {
    $('#mgModalContainer select:not(.ajax)').each(function () {
        ...
        if ($(this).attr('bi-event-change')) {
            ...
            selectizeOptions.onItemAdd = function (value) {
        ...
        if ($(this).attr('bi-event-dropdown-close')) {
            var biEventAction = $(this).attr('bi-event-dropdown-close');
            selectizeOptions.plugins = ['remove_button'];
            selectizeOptions.onDropdownClose = function (dropdown) {
                if (biEventAction && typeof mgPageControler[biEventAction] === "function") {
                    setTimeout(function () {
                        mgPageControler[biEventAction]();
                    }, 500);
...
function updateNodesSelect()
{
    mgEventHandler.runCallback('SelectFieldValueChanged', 'sid');
}