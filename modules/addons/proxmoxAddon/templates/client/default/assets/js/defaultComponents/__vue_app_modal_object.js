    ...
    methods: {
        ...
        addMassActionsToData: function (formData) {
            if (mgPageControler.vueLoader.massActionIds) {
                formData.massActions = mgPageControler.vueLoader.massActionIds;
                return formData;
            } else {
                return formData;
            }
        ...
        runRefreshActions: function () {
            var self = this;
            if (self.refreshingState.length > 0) {
                $.each(self.refreshingState, function (index, Id) {
                    var targetId = Id;
                    self.$nextTick(function () {
                ...
                $.post(self.targetUrl, formData)
                        .done(function (data) {
                            ...
                            if (data.status === 'success') {
                                ...
                                mgPageControler.vueLoader.runRefreshActions((data && typeof data.refreshIds !== undefined) ? data.refreshIds : null);
                                self.cleanMassActions();
        ...
        handleErrorMessage: function (data) {
            var self = this;
            self.addAlert(data.status, (data.message ? data.message : 'Error Code: ' + data.data.errorCode
                    + '<br> Error Token: ' + data.data.errorToken + '<br> Error Time: ' + data.data.errorTime
                    + '<br> Message: ' + data.data.errorMessage));
