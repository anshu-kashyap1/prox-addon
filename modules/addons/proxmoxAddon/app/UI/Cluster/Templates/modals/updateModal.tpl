<div class="lu-modal show" id="confirmationModal">
    <div class="lu-modal__dialog">
        <div class="lu-modal__content" id="mgModalContainer">
            ...
            {assign var="updateForm" value=$rawObject->getForm('nodeUpdateForm')}
            <div class="lu-modal__nav">
                <ul class="lu-nav lu-nav--md lu-nav--h lu-nav--tabs">
                    {assign var="sectArrKeys" value=$updateForm->getSections()|array_keys}
                    {foreach from=$updateForm->getSections() key=sectionID  item=section }
            ...
            <div class="lu-modal__body">
                <form id="{$updateForm->getId()}" namespace="{$updateForm->getNamespace()}" index="{$updateForm->getIndex()}" mgformtype="{$updateForm->getFormType()}">
                    {foreach from=$updateForm->getFields() item=field }
                        {$field->getHtml()}
                    {/foreach}
